...
# Arrow stats (set by player when spawned)
...
func _physics_process(delta: float) -> void:
	...
	# Homing behavior (if upgraded)
	if homing_strength > 0:
		var nearest_enemy = find_nearest_enemy()
		if nearest_enemy:
			var to_enemy = (nearest_enemy.global_position - global_position).normalized()
			direction = direction.lerp(to_enemy, homing_strength * delta * 5.0)  # 5x multiplier for noticeable effect
			rotation = direction.angle()  # Update rotation to face new direction
